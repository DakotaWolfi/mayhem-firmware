...
void EF28FoxHuntView::set_config_page(bool enabled) {
    // Force visible text regions to overwrite themselves before page widgets are hidden.
    // This avoids stale characters from the previous page on PortaPack displays.
    if (enabled) {
        text_status.set("                                ");
        text_found.set("                                ");
        text_target.set("                                ");
